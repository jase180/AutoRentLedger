...
def test_daily_operation_checks_schema_then_backs_up_then_syncs(tmp_path):
    ...
    def backup_operation(path, *, now, default_directory):
        calls.append(("backup", path, now, default_directory))
    ...
    assert result == DailyOperationResult(backup_path, sync_result())
    assert [call[0] for call in calls] == ["schema", "backup", "sync"]
    assert calls[1][2] == now
    assert calls[1][3] == tmp_path / "backups"
...
def test_daily_cli_forwards_sync_inputs_after_backup(monkeypatch, capsys):
    ...
    def execute_daily(database, backup_dir, sync_operation):
        captured["daily"] = (database, backup_dir)
        return DailyOperationResult(backup_dir / "verified.db", sync_operation())
...
def test_daily_failures_are_safe_and_have_distinct_stage_output(monkeypatch, capsys):
    def fail_backup(database, backup_dir, sync_operation):
    ...
    def fail_sync(database, backup_dir, sync_operation):
...
def test_repeated_daily_runs_create_separate_backups_without_duplicate_evidence(tmp_path):
    ...
    assert after[2] == CURRENT_SCHEMA_VERSION == 8
