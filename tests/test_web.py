...
def populate_dashboard(database_path, raws, payments, payers, rentals, obligations, allocations):
    ...
    add_payment(raws, payments, 5, 1000, date(2026, 7, 20), "UNKNOWN SENDER")
    ...
    create_rent_schedule(
        web_routes.SQLiteRentScheduleRepository(database_path),
        missing_account.id,
        "1450.00",
        1,
        "2026-09-15",
...
def test_route_uses_canonical_builder_and_configured_database(tmp_path, monkeypatch):
    ...
    original_builder = web_routes.build_owner_overview
    captured_paths = []

    def recording_builder(*args):
        captured_paths.extend(repository.database_path for repository in args[:-1])
        return original_builder(*args)

    monkeypatch.setattr(web_routes, "build_owner_overview", recording_builder)
    response = create_app(database_path).test_client().get(
        "/overview?period=2026-09"
    )

    assert response.status_code == 200
    assert captured_paths == [database_path] * 5
...
def test_domain_failure_is_safe_and_does_not_call_gmail(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(web_routes, "build_owner_overview", fail_builder)
